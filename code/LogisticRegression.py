...
class logistic_regression(object):
    ...
    def fit_BGD(self, X, y):
        ...
        for i in range(self.max_iter):
            g = np.mean([self._gradient(x, y) for x, y in zip(X,y)], 0) 
    ...
    def fit_SGD(self, X, y):
        ...
        self.W = np.zeros((n_features,))
        for i in range(self.max_iter):
            rand_i = np.random.randint(0,n_samples)
            g = self._gradient(X[rand_i],y[rand_i]) 
            self.W += self.learning_rate*(-g)
		### END YOUR CODE
        return self
    ...
    def predict_proba(self, X):
		...
		### YOUR CODE HERE
        #n_samples, n_features = X.shape
        predicted_prob = np.array([1/(1+np.exp(np.dot(self.W,x))) for x in X]) 
        predicted_prob = np.stack((predicted_prob, 1-predicted_prob),axis=-1) 
    ...
    def predict(self, X):
		...
		### YOUR CODE HERE
        #n_samples, n_features = X.shape
        prediction = np.array([-1 if np.dot(self.W,x) < 0 else 1 for x in X])
        return prediction
		### END YOUR CODE
    ...
    def score(self, X, y):
        ...
        score = (np.sum(y==prediction)/n_samples)*100
        return score
		### END YOUR CODE
